...
iniget() 
{
    ...
    $GAWK -F= -v printtype=$INIPRINTTYPE -v line="$INILINE" -v way=$INIWLINE '
...
$GAWK -F= -v del=$ini_del -v textfile=/tmp/.file.$$ '
  BEGIN {
...
{
  Z=`ps -af | grep "VTOC_Linux.sh" | grep -v "grep" | wc -l`
  #Determine if the VTOC_Linux.sh already running
  if [[ "$Z" -ne 2 ]] ; then
    echo "Another instance of VTOC_Linux.sh is running. Please wait for it to finish first and then try again."
    exit 2
  fi
}
...
function MergeAllReport()
 {
	...
	cd $COMMONDIR
	$GAWK 'FNR==1 && NR!=1{next;}{print}' *.csv > COMMONREPORT.csv
...

{
	...
	cd   $DIRECTORY_CSV
	$GAWK 'FNR==1 && NR!=1{next;}{print}' *.csv > IN.csv
	chmod 777 IN.csv
	$GAWK -F, '{$1="CELL" FS $1;}1' OFS=, IN.csv > CELL.csv	
...
function ReportGenerationForCellRelation()
{
	...
	$GAWK 'FNR==1 && NR!=1{next;}{print}' *.csv > IN.csv
	chmod 777 IN.csv
	$GAWK -F, '{$1="CELLRELATION" FS $1;}1' OFS=, IN.csv > CELLRELATION.csv
...
function ReportGenerationForProcessorLoad()
{
	...
	$GAWK 'FNR==1 && NR!=1{next;}{print}' *.csv > IN.csv
	chmod 777 IN.csv
	$GAWK -F, '{$1="PROCESSORLOAD" FS $1;}1' OFS=, IN.csv > PROCESSORLOAD.csv
...
function ReportGenerationForTermPointToMME()
{
	...
	$GAWK 'FNR==1 && NR!=1{next;}{print}' *.csv > IN.csv
	chmod 777 IN.csv
	$GAWK -F, '{$1="TERMPOINTTOMME" FS $1;}1' OFS=, IN.csv > TERMPOINTTOMME.csv
...
function ReportGenerationForTermPointToENB()
{
	...
	$GAWK 'FNR==1 && NR!=1{next;}{print}' *.csv > IN.csv
	chmod 777 IN.csv
	$GAWK -F, '{$1="TERMPOINTTOENB" FS $1;}1' OFS=, IN.csv > TERMPOINTTOENB.csv
...

{
	...
	$GAWK 'FNR==1 && NR!=1{next;}{print}' *.csv > IN.csv
	chmod 777 IN.csv
	$GAWK -F, '{$1="ENODEB" FS $1;}1' OFS=, IN.csv > ENODEB.csv
...
{
    ${ECHO} -e "\t\tModifying db.cfg and new_table.cfg  started\n" | $TEE -a ${LOGFILE}
	...
	#${PERL} -pi -e '~s/'${PREVIOUS_EVENT_TABLE_NAME}_'/'${CTRS_TABLE}_'/g' ${NEW_TABLE_CFG} 
	#${PERL} -pi -e '~s/'${PREVIOUS_COUNTER_TABLE_NAME}'/'${COUNTER_TABLE}'/g' ${NEW_TABLE_CFG}
...
rm -rf ${NEW_TABLE_CFG}
    ...
    chmod -R 777 ../etc     
    ${ECHO} -e "\t\tdb.cfg and new_table.cfg modified successfully!!!\n" | ${TEE} -a ${LOGFILE}

 }
 
 function ModifySqlQueries()
{  
   ${ECHO} -e "\t\tModifying Sql Queries\n" | $TEE -a ${LOGFILE}
    
	echo ${PREVIOUS_EVENT_TABLE_NAME}
	echo ${CTRS_TABLE}
    PT="${PREVIOUS_TOPOLOGY_TABLE_NAME}_"
	TN="${TOPOLOGY_TABLE}_"
echo $PT
echo $TN	
	...
	${PERL} -pi -e '~s/'${PREVIOUS_EVENT_TABLE_NAME}_'/'${CTRS_TABLE}_'/g' ../query/${TEST_RELEASE}/MultiEventBasedCounter/*.sql
     ...
     chmod -R 777 ../query
    ${ECHO} -e "\t\tQueries modified successfully!!!\n" | ${TEE} -a ${LOGFILE}
 }
 
 
  function loadSchema()
 {
	${ECHO} -e "\t\tSchema $SCHEMA Loading Started!!!\n" | ${TEE} -a ${LOGFILE}
	${JAVA} -jar  ${GRIT_PATH} -a 10dash -t ${SCHEMA} -s CTRS -f CTRS -p ${CTRS_TABLE}  2>> ${LOGFILE}
	${ECHO} -e  "\t\tSchema $SCHEMA Loading Finished!!!\n" | ${TEE} -a ${LOGFILE}
 }

 function createTable()
 {    
	${ECHO} -e "\t\tCreating Event Tables...\n" | ${TEE} -a ${LOGFILE}    
    ${JAVA} -jar ../lib/Database.jar ./ ${DATABASE_CFG} ${SET_VERBOSE} ${TIME_START}  2>> ${LOGFILE}
	${ECHO}  -e "\t\tTable Creation Finished\n" | ${TEE} -a ${LOGFILE}
 }
 
 function parseRawData()
{   
    DIRECTORY="Sql"
    ${ECHO} -e   "\t\tParsing Raw Data Started!!!\n" | ${TEE} -a ${LOGFILE}
    cd ${CTRS_FILES}
    if [ ! -d $DIRECTORY ]; then
        mkdir $DIRECTORY
    fi
    ...
    ${ECHO} -e   "\t\tParsing Raw Data Finished!!!\n" | ${TEE} -a ${LOGFILE}
...
{ 
    ...
    ${ECHO} -e "\t\tPopulating Topology Table...\n" | ${TEE} -a ${LOGFILE}
...
{      
    ...
    ${ECHO} -e "\t\tCounter Table" $COUNTER_TABLE" Populated Successfully!!!\n" | ${TEE} -a ${LOGFILE}
 }
 
 function MatchRulesForCell()
 { 
    ${ECHO} -e  "\t\tRules Matching Started!!!\n" | ${TEE} -a ${LOGFILE}    
	...
	if [ ! -d $DIRECTORY_CSV ]; then
     ...
     for cellrules in ../rules/${TEST_RELEASE}/CELL/*.rules;   
	    ...
	    ${JAVA} -jar ${GRIT_PATH} -a rules -r ../rules/${TEST_RELEASE}/CELL/$eventid.rules -d ${DATABASE_CFG} -t ${NEW_TABLE_CFG}  -v -N -e -csv ../results/${TEST_RELEASE}/CELL/CSV/$eventid.csv 2>> ${LOGFILE}  > ../results/${TEST_RELEASE}/CELL/TEXT/$eventid.txt                      
                        ...
                        if [[ $?  == 1 ]]
                            then
                                ${ECHO} -e "\t\t Rule Failed for "$eventid "\n" | $TEE -a ${LOGFILE}
                            else
                                ${ECHO} -e "\t\t Rule run successfully for "$eventid "\n" | $TEE -a ${LOGFILE}
								
                        fi
                      
        done  
${ECHO} -e "\t\tRules Matching done For CELL!!! You can Check Event Wise Results in results folder\n" | ${TEE} -a ${LOGFILE}
...
${ECHO} -e "\t\tRules Matching done for CELL_RELATION!!! You can Check Event Wise Results in results folder\n" | ${TEE} -a ${LOGFILE}
	...
	if [ ! -d $DIRECTORY_CSV ]; then
     ...
     for cellrules in ../rules/${TEST_RELEASE}/PROCESSORLOAD/*.rules;   
		...
		${JAVA} -jar ${GRIT_PATH} -a rules -r ../rules/${TEST_RELEASE}/PROCESSORLOAD/$eventid.rules -d ${DATABASE_CFG} -t ${NEW_TABLE_CFG}  -v -N -e -csv ../results/${TEST_RELEASE}/PROCESSORLOAD/CSV/$eventid.csv 2>> ${LOGFILE}  > ../results/${TEST_RELEASE}/PROCESSORLOAD/TEXT/$eventid.txt 
			...
			${ECHO} -e "\t\tRules Matching done for PROCESSORLOAD!!! You can Check Event Wise Results in results folder\n" | ${TEE} -a ${LOGFILE}
...
${ECHO} -e "\t\tRules Matching done for TERMPOINTTOMME!!! You can Check Event Wise Results in results folder\n" | ${TEE} -a ${LOGFILE}
...
${ECHO} -e  "\t\tRules Matching done Successfully For TERMPOINTTOENB!!! You can Check Event Wise Results in results folder\n" | ${TEE} -a ${LOGFILE}
...
${ECHO}  -e "\t\tRules Matching done Successfully for ENODEB!!! You can Check Event Wise Results in results folder\n" | ${TEE} -a ${LOGFILE}
